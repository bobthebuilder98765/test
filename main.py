...
class GameButtonView(View):
    def __init__(self):
        ...
        for game, game_data in games.items():
            self.add_item(Button(style=game_data['style'], label=game, custom_id=game, emoji=game_data['emoji']))
    ...
    if interaction.data['component_type'] == 2:  # 2 corresponds to a button
        ...
        if interaction.user.voice:
            ...
            if role:
                ...
                message = await channel.send(f"Role {role_name} not found. {interaction.user.mention}")
                await asyncio.sleep(10)
                await message.delete()
        else:
